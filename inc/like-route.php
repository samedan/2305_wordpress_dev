<?php 
   ...
   function deleteLike($data) {
      $likeId = sanitize_text_field($data['like']); // id of teh post to delete
      if(get_current_user_id() == get_post_field('post_author', $likeId) 
          AND 
        get_post_type($likeId) == "like") 
        {
          wp_delete_post($likeId, true); // 'true' means skip trash
          return 'Congrats, like deleted';
      } else {
        die("You do not have permission to dlete that.");
      }
